...
static void inode_locate(struct partition* part, uint32_t inode_no, struct inode_position* inode_pos) {
    ...
    // 待查找的 inode 所在扇区中的起始地址（从此位置起到一个inode的大小）
    uint32_t off_size_in_sec = off_size % 512;

    /* 判断此 i 结点是否跨越 2 个扇区 */ 
    uint32_t left_in_sec = 512 - off_size_in_sec;
    // 若扇区内剩下的空间不足以容纳一个inode ，必然是 i 结点跨越了 2 个扇区
...
void inode_sync(struct partition* part, struct inode* inode, void* io_buf) {
    ...
    /* 硬盘中的 inode 中的成员inode_tag 和 i_open_cnts 是不需要的，
...
void inode_delete(struct partition* part, uint32_t inode_no, void* io_buf) {
    ...
    if (inode_pos.two_sec) {
        ...
        ide_write(part->my_disk, inode_pos.sec_lba, inode_buf, 2);  // 清0后覆盖
    // 未跨扇区，只读入 1 个扇区就好
    } else {
        ide_read(part->my_disk, inode_pos.sec_lba, inode_buf, 1);
        memset((inode_buf + inode_pos.off_size), 0, sizeof(struct inode));
        ide_write(part->my_disk, inode_pos.sec_lba, inode_buf, 1);  // 清0后覆盖
    }
}
...
void inode_release(struct partition* part, uint32_t inode_no) {
    ...
    /* c inode 所有的块地址已经收集到 all blocks 中，下面逐个回收 */
    block_idx = 0;
    ...
    /* 以下 inode_delete 是调试用的 
       此函数会在inode_table 中将此 inode 清 0,
