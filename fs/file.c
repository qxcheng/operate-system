...
int32_t pcb_fd_install(int32_t global_fd_idx) {
    ...
    while (local_fd_idx < MAX_FILES_OPEN_PER_PROC) {  // =8
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
    ...
    // 若有增量，便涉及到分配新扇区及是否分配一级间接块表，下面要分三种情况处理
        ...
        // 第二种情况：旧数据在 12 个直接块内，新数据将使用间接块
            ...
            block_idx = file_has_used_blocks - 1;  // 指向旧数据所在的最后－个扇区
            all_blocks[block_idx] = file->fd_inode->i_sectors[block_idx];
        ...
        // 第三种情况：新数据占据间接块
            ...
            while (block_idx < file_will_use_blocks) {
                block_lba = block_bitmap_alloc(cur_part);
                if (block_lba == -1) {
                    printk("file_write: block_bitmap_alloc for situation 4 failed\n");
                    return -1;
                }
                all_blocks[block_idx++] = block_lba;
                // 每分配一个块就将位图同步到硬盘
    ...
    while (bytes_written < count) {
        ...
        sec_idx = file->fd_inode->i_size / BLOCK_SIZE;  // 应写入的扇区下标
        sec_lba = all_blocks[sec_idx];  // 应写入的扇区地址
        sec_off_bytes = file->fd_inode->i_size % BLOCK_SIZE;  // 在扇区内的偏移量
        sec_left_bytes = BLOCK_SIZE - sec_off_bytes;  // 该扇区剩余的空间
