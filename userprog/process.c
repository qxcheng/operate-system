...
void start_process(void* filename_) {
    ...
    cur->self_kstack += sizeof(struct thread_stack);  // 指向了中断栈起始地址 
    ...
    asm volatile ("movl %0, %%esp; jmp intr_exit" : : "g" (proc_stack) : "memory");  // 从中断号开始弹栈 
...
void process_activate(struct task_struct* p_thread) {
    ...
    if (p_thread->pgdir) {
        update_tss_esp(p_thread);  // 更新该进程的 esp0 ，用于此进程被中断时保留上下文
    }
}
...
void process_execute(void* filename, char* name) { 
    ...
    block_desc_init(thread->u_block_desc);  // 用户内存块描述符数组的初始化
