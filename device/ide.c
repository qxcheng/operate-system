...
static void select_sector(struct disk* hd, uint32_t lba, uint8_t sec_cnt) {
    ...
    /* 写入 lba 地址，即扇区号 */
    outb(reg_lba_l(channel), lba);  // lba 地址的低8位，不用单独取出低8位, outb 函数中的汇编指令 outb %b0, %w1 会只用 al
...
static void read_from_sector(struct disk* hd, void* buf, uint8_t sec_cnt) {
    ...
    insw(reg_data(hd->my_channel), buf, size_in_byte / 2);  // 转换成字
}
...
static bool busy_wait(struct disk* hd) {
    ...
    while (time_limit -= 10 >= 0) {
        if  (!(inb(reg_status(channel)) & BIT_STAT_BSY)) {
            return (inb(reg_status(channel)) & BIT_STAT_DRQ);  // DRQ 位为 1 表示硬盘己经准备好数据了
        } else {
...
void ide_write(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt) {
    ...
    lock_release(&hd->my_channel->lock);  // 醒来后开始释放锁
}
...
static void identify_disk(struct disk* hd) {
    ...
    read_from_sector(hd, id_info, 1);  // 从硬盘获取信息到 id_info 
...
void ide_init() {
    ...
    channel_cnt = DIV_ROUND_UP(hd_cnt, 2);  // 一个 ide 通道上有两个硬盘，根据硬盘数量反推有几个ide通道
    ...
    while (channel_no < channel_cnt) {
        ...
        case 0:
            ...
            channel->irq_no = 0x20 + 14; // 从片 8259a 上倒数第二的中断引脚, 硬盘，也就是 ide0 通道的中断向量号
            break;
        case 1:
            channel->port_base = 0x170;  // ide1 通道的起始端口号是 0x170
            channel->irq_no = 0x20 + 15; // 从片 8259A 上的最后一个中断引脚，我们用来响应 ide1 通道上的硬盘中断
            break;
        }
        ...
        /* 始化为 0 ，目的是向硬盘控制器请求数据后，硬盘驱动 sema_down 此信号量会
           阻塞线程，直到硬盘完成后通过发中断，由中断处理程序将此信号量 
           sema_up ，唤醒线程 */
